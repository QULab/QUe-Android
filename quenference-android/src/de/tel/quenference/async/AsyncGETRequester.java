...
/**
 * Represents a async GET requester. 
 * For given URL's the AsyncTask will execute GET requests and 
 * returns the result of each request as JSONObject in given PostExecuteJob
 * doJob method call. Means to executed the AsyncTask, the caller needs to
 * pass into the task the URL's to request and a PostExecuteJob Object which will
 * be used to return the values or used if an exception appears.
 * If the Task was successful the doJob Method will be called of the 
        ...
        try {
          ...
          if (response == null || statusCode >= 400) {
            ...
            if (responseEtag != null) {
              ...
              if (newEtag != null && !newEtag.equalsIgnoreCase(etag))
                job.handleNewEtag(url, newEtag);
            }
          }
